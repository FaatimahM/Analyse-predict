...
def dictionary_of_metrics(items):
    ...
    '''
    assert analysepredict.dict_metrics(gauteng) == {'mean': 26244.42, 
...
def five_num_summary(items):
    ...
    '''
    assert analysepredict.five_num_summary(gauteng) == {'max': 39660.0,
...
def date_parser(dates):
    ...
    ''' 
    assert analysepredict.date_parser(dates[:3]) == ['2019-11-29', '2019-11-29', '2019-11-29'], 'incorrect'
    assert analysepredict.date_parser(dates[-3:]) == ['2019-11-29', '2019-11-29', '2019-11-29'], 'incorrect'
...
def extract_municipality_hashtags(df):
    ...
    ''' 
    assert analysepredict.extrct_hashtags(twitter_df.copy()).loc[4,'hashtags'] == ['#eskomfreestate', '#mediastatement'], 'incorrect'
...
def word_splitter(df):
    ...
    ''' 
    assert analysepredict.word_split(twitter_df.copy()).loc[0,'Split Tweets'] == ['@bongadlulane',
...
def stop_words_remover(df):
    ...
    '''
    assert analysepredict.words_remover(twitter_df.copy()).loc[0,'Without Stop Words'] == ['@bongadlulane', 
